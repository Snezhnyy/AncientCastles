 ...
 * @author Snowdrop
 */
public class HealAction extends GeneralAction{
    Image magicTile;
    ...
    public HealAction(Room room, GameJFrame frame, Point cursorTile, UnitDescription ud, Point pos) {
        selected = true;
        magicTile = new ImageIcon("src" + File.separator + "projectone"+File.separator+"Image"+File.separator+"Cursor"+File.separator+"magicGP.png").getImage();
        activeUnit = ud;
    ...
    private void healAction(Point cursorTile, Room room) {
        if(attackMap[cursorTile.x][cursorTile.y] > -1 && room.units[cursorTile.x][cursorTile.y] != null){
            if(room.units[cursorTile.x][cursorTile.y].imageId == 9 || room.units[cursorTile.x][cursorTile.y].imageId == 10){
            ...
            if(room.units[cursorTile.x][cursorTile.y].owner == activeUnit.owner) {
                if(room.units[cursorTile.x][cursorTile.y].wolfDamage > 0)
                    room.units[cursorTile.x][cursorTile.y].wolfDamage -=15;
                if(room.units[cursorTile.x][cursorTile.y].wolfDamage < 0) room.units[cursorTile.x][cursorTile.y].wolfDamage = 0;
                room.units[cursorTile.x][cursorTile.y].heal(30);
                activeUnit.canAttack = false;
            }
            else {
                room.units[cursorTile.x][cursorTile.y].loseHP(20);
                if(room.units[cursorTile.x][cursorTile.y].hp < 1)
                    room.units[cursorTile.x][cursorTile.y] = null;
                activeUnit.canAttack = false;
            }
        }
